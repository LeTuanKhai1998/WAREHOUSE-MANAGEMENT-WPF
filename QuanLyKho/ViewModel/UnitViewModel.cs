...
namespace QuanLyKho.ViewModel
{
    class UnitViewModel : BaseViewModel
    {
        ...
        private void SelectAll(bool? select, ObservableCollection<Unit> models)
        {
        ...
        public UnitViewModel(string _idHeader, string _displayNameHeader)
        {
            ...
            progressBarViewModel.Title = "Đang tải danh sách đơn vị tính, vui lòng chờ...";
            ...
            AddCommand = new RelayCommand<Category>((p) => { return true; }, (p) =>
            {
                ...
                addViewModel.NameTitle = "Tên đơn vị tính:";
                addViewModel.Title = "Thêm đơn vị tính";
            ...
            {
                Unit ca = List.Where(i => i.IsSelected == true).ToArray()[0];
                AddViewModel addViewModel = new AddViewModel();
                AddView addView = new AddView();
                ...
                addView.DataContext = addViewModel;
                addViewModel.NameTitle = "Tên đơn vị tính:";
                addViewModel.Title = "Sửa đơn vị tính";
                addViewModel.NameTable = "Unit";
                addView.ShowDialog();
                List.Clear();
                loadData();
            ...
            {
                Unit objXoa = kiemTraChonXoa();

                string tex = "";
                int count = List.Count(x => x.IsSelected == true);
                if (count == 1)
                    tex = "Hệ thống sẽ xóa hoàn toàn đơn vị tính " + objXoa.DisplayName + ". Bạn có chắc chắn muốn xóa?";
                else
                    tex = "Hệ thống sẽ xóa hoàn toàn " + count + " đơn vị tính bạn đã chọn. Bạn có chắc chắn muốn xóa?";
                ...
                if (editViewModel.Result == true)
                {
                    ...
                    foreach (Unit ca in List)
                        if (ca.IsSelected == true)
                            try
                            {
                                string texSql = "exec usp_Delete_Unit " + ca.Id;
                    ...
                    else
                    {
                        ...
                        _toast.ShowSuccess("Đã xóa thành công danh sách đơn vị tính đã chọn!");
        ...
        private void loadData()
        {
            ...
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select * from uv_View_Unit", con);
                adapter = new SqlDataAdapter(cmd);
                ds = new DataSet();
                adapter.Fill(ds);
                if (List == null)
                    List = new ObservableCollection<Unit>();
