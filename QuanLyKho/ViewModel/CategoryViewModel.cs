...
namespace QuanLyKho.ViewModel
{
    class CategoryViewModel : BaseViewModel
    {
        ...
        private void SelectAll(bool? select, ObservableCollection<Category> models)
        {
        ...
        public CategoryViewModel(string _idHeader, string _displayNameHeader)
        {
            ...
            progressBarViewModel.Title = "Đang tải danh sách loại hàng, vui lòng chờ...";
            ...
            {
                Category ca = List.Where(i => i.IsSelected == true).ToArray()[0];
                AddViewModel addViewModel = new AddViewModel();
                AddView addView = new AddView();
                ...
                addView.DataContext = addViewModel;
                addViewModel.NameTitle = "Tên loại hàng:";
                addViewModel.Title = "Sửa loại hàng";
                addViewModel.NameTable = "Category";
                addView.ShowDialog();
                List.Clear();
                loadData();
            ...
            {
                Category objXoa = kiemTraChonXoa();

                string tex = "";
                int count = List.Count(x => x.IsSelected == true);
                if (count == 1)
                    tex = "Hệ thống sẽ xóa hoàn toàn loại hàng " + objXoa.DisplayName + ". Bạn có chắc chắn muốn xóa?";
                else
                    tex = "Hệ thống sẽ xóa hoàn toàn " + count + " loại hàng bạn đã chọn. Bạn có chắc chắn muốn xóa?";
                ...
                if (editViewModel.Result == true)
                {
                    ...
                    foreach (Category ca in List)
                        if (ca.IsSelected == true)
                            try
                            {
                                string texSql = "exec usp_Delete_Category " + ca.Id;
                    ...
                    else
                    {
                        ...
                        _toast.ShowSuccess("Đã xóa thành công danh sách loại hàng đã chọn!");
        ...
        private void loadData()
        {
            ...
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select * from uv_View_Category", con);
